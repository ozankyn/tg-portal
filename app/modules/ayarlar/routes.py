...
from app.models.ik import Departman, Pozisyon, Calisan
...
# -*- coding: utf-8 -*-
"""
TG Portal - Departman Yönetimi Routes
Bu kodu app/modules/ayarlar/routes.py dosyasının SONUNA ekle
Ayrıca dosyanın başındaki import'lara şunu ekle:
    from app.models.ik import Departman, Pozisyon, Calisan
    from app.models.base import CalisanDurumu
"""
...
def departman_liste():
    ...
    departmanlar = Departman.query.filter_by(is_deleted=False).order_by(Departman.ad).all()
    
    return render_template('ayarlar/departman_liste.html', departmanlar=departmanlar)


@ayarlar_bp.route('/departman/ekle', methods=['GET', 'POST'])
@login_required
def departman_ekle():
    """Yeni departman ekle"""
    if not current_user.is_admin:
    ...
    if request.method == 'POST':
        departman = Departman(
            ...
            ust_departman_id=int(request.form.get('ust_departman_id')) if request.form.get('ust_departman_id') else None,
        ...
        flash(f'{departman.ad} departmanı oluşturuldu.', 'success')
        return redirect(url_for('ayarlar.departman_liste'))
    ...
    return render_template('ayarlar/departman_form.html',
                          departman=None,
                          departmanlar=departmanlar,
                          calisanlar=calisanlar)


@ayarlar_bp.route('/departman/<int:id>', methods=['GET', 'POST'])
@login_required
def departman_duzenle(id):
    ...
    if request.method == 'POST':
        departman.ad = request.form.get('ad', '').strip()
        departman.kod = request.form.get('kod', '').strip().upper() or None
        departman.aciklama = request.form.get('aciklama', '').strip() or None
        departman.ust_departman_id = int(request.form.get('ust_departman_id')) if request.form.get('ust_departman_id') else None
        departman.yonetici_id = int(request.form.get('yonetici_id')) if request.form.get('yonetici_id') else None
        departman.aktif = request.form.get('aktif') == 'on'
    ...
    # Kendisi hariç departmanlar (üst departman için)
    departmanlar = Departman.query.filter(
        Departman.is_deleted == False,
        Departman.aktif == True,
        Departman.id != id
    ).order_by(Departman.ad).all()
...
@ayarlar_bp.route('/departman/<int:id>/sil', methods=['POST'])
@login_required
def departman_sil(id):
    ...
    # Bu departmanda çalışan var mı?
    calisan_sayisi = Calisan.query.filter_by(departman_id=id, is_deleted=False).count()
    if calisan_sayisi > 0:
        flash(f'Bu departmanda {calisan_sayisi} çalışan var. Önce çalışanları başka departmana taşıyın.', 'warning')
        return redirect(url_for('ayarlar.departman_liste'))
    
    departman.is_deleted = True
    departman.aktif = False
    db.session.commit()
...
def pozisyon_liste():
    ...
    pozisyonlar = Pozisyon.query.filter_by(is_deleted=False).order_by(Pozisyon.ad).all()
    
    return render_template('ayarlar/pozisyon_liste.html', pozisyonlar=pozisyonlar)


@ayarlar_bp.route('/pozisyon/ekle', methods=['GET', 'POST'])
@login_required
def pozisyon_ekle():
    """Yeni pozisyon ekle"""
    if not current_user.is_admin:
    ...
    if request.method == 'POST':
        pozisyon = Pozisyon(
            ...
            seviye=int(request.form.get('seviye')) if request.form.get('seviye') else None,
            aciklama=request.form.get('aciklama', '').strip() or None,
            aktif=request.form.get('aktif') == 'on'
        )
        
        db.session.add(pozisyon)
        db.session.commit()
        
        flash(f'{pozisyon.ad} pozisyonu oluşturuldu.', 'success')
...
@ayarlar_bp.route('/pozisyon/<int:id>', methods=['GET', 'POST'])
@login_required
def pozisyon_duzenle(id):
    """Pozisyon düzenle"""
    if not current_user.is_admin:
        flash('Bu sayfaya erişim yetkiniz yok.', 'danger')
        return redirect(url_for('core.dashboard'))
    
    pozisyon = Pozisyon.query.get_or_404(id)
    
    if request.method == 'POST':
        pozisyon.ad = request.form.get('ad', '').strip()
        pozisyon.kod = request.form.get('kod', '').strip().upper() or None
        pozisyon.departman_id = int(request.form.get('departman_id')) if request.form.get('departman_id') else None
        pozisyon.seviye = int(request.form.get('seviye')) if request.form.get('seviye') else None
        pozisyon.aciklama = request.form.get('aciklama', '').strip() or None
        pozisyon.aktif = request.form.get('aktif') == 'on'
